...
const starterMessages = [
  {
    seedKey: 'menemen-intro',
    context: 'Menemen',
    sender: 'assistant',
    text:
      'I see you are making Menemen with four ingredients. Want me to adapt it to your taste?'
  },
  {
    seedKey: 'menemen-user-request',
    context: 'Menemen',
    sender: 'user',
    text: 'Make it spicier.'
  },
  {
    seedKey: 'menemen-chef-reply',
    context: 'Menemen',
    sender: 'assistant',
    text: 'Done. Add chili flakes and a little extra garlic near the end.'
  }
];
...
async function seedMessages() {
  await AssistantMessage.bulkWrite(
    starterMessages.map((message) => ({
      updateOne: {
        filter: { seedKey: message.seedKey },
        update: { $set: message },
        upsert: true
      }
    }))
  ...
  await AssistantMessage.deleteMany({
    ...
    text: {
      $in: [
        'I see you are making Menemen. Want help adjusting the recipe?',
        'Make it spicier.',
        'Add chili flakes and a little extra garlic near the end.'
      ]
    }
  });
}
