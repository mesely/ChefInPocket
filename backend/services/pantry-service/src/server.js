...
const starterGroceryItems = [
  { itemId: 'g1', title: 'Eggs', note: 'Need 2 more', emoji: '🥚' },
  { itemId: 'g2', title: 'Tomatoes', note: 'Need 4 more', emoji: '🍅' },
  { itemId: 'g3', title: 'Feta', note: 'Need 1 pack', emoji: '🧀' }
];
...
async function seedPantry() {
  await PantryIngredient.bulkWrite(
    starterIngredients.map((ingredient) => ({
      updateOne: {
        filter: { ingredientId: ingredient.ingredientId },
  ...
  await GroceryItem.deleteMany({
    itemId: { $exists: false },
    title: { $in: starterGroceryItems.map((item) => item.title) }
  });
}
...
app.post('/match', async (req, res) => {
  ...
  if (selected.includes('eggs') && selected.includes('tomato')) {
    recommendations.push('menemen');
  }

  if (selected.includes('eggs') && selected.includes('lettuce')) {
    recommendations.push('egg-salad');
  }
...
app.delete('/grocery-list/:id', async (req, res) => {
